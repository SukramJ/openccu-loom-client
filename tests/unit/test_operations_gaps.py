...
plus the ``hub.install_mode_changed`` broadcast binding — against
aioresponses so the request shapes are pinned to the daemon contract.
"""
...
class TestSchedulesOperations:
    async def test_get_channel_week_profile(self, http) -> None:
        t, mock = http
        mock.get(
            f"{_BASE}/devices/VCU1/channels/1/week_profile",
            payload={
                "address": "VCU1:1",
...
class TestLinksOperations:
    ...
    async def test_remove_link_passes_query(self, http) -> None:
        t, mock = http
        mock.delete(
            f"{_BASE}/devices/VCU1/links?sender=VCU1:1&receiver=VCU2:1",
            status=202,
        )
    ...
    async def test_enable_central_links(self, http) -> None:
        t, mock = http
        mock.post(f"{_BASE}/devices/VCU1/central-links", status=202)
...
class TestDevicesOperationsGaps:
    async def test_update_firmware_does_not_retry(self, http) -> None:
        t, mock = http
        mock.post(f"{_BASE}/devices/VCU1/firmware/update", status=202)
    ...
    async def test_list_calculated_data_points(self, http) -> None:
        t, mock = http
        mock.get(
            f"{_BASE}/devices/VCU1/channels/1/calc-dps",
