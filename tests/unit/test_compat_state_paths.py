...
def test_device_state_path_format() -> None:
    assert device_state_path(address="vcu1769958", channel=3, parameter="state") == "device/status/VCU1769958/3/STATE"
...
def test_device_state_path_roundtrip() -> None:
    ...
    assert parse_device_state_path(state_path=path) == ("VCU1769958", 3, "STATE")


def test_sysvar_state_path_roundtrip() -> None:
    path = sysvar_state_path(name="Presence")
    assert parse_sysvar_state_path(state_path=path) == "Presence"
...
def test_parse_device_rejects_non_device_paths(bad: str) -> None:
    assert parse_device_state_path(state_path=bad) is None
...
def test_parse_sysvar_rejects_non_sysvar_paths(bad: str) -> None:
    assert parse_sysvar_state_path(state_path=bad) is None
