...
openccu-loom-client. It is modeled on the
...
gates: the test suite (`pytest tests/`) and the pre-commit hooks (`prek run --all-files`, which
runs ruff, ruff-format, mypy, pylint, codespell, bandit, yamllint, and prettier). AI assistance
does not lower the bar.

## How this project itself uses AI

In the interest of transparency: openccu-loom-client is partly developed with AI assistance
